...
SuchThatClause::SuchThatClause(const SuchThatClauseType& t,
                               const QueryEntity& f, const QueryEntity& s)
...
bool SuchThatClause::equals(const QueryClause& other) const {
    if (const SuchThatClause* ptr =
            dynamic_cast<const SuchThatClause*>(&other)) {
        return type == ptr->type && firstArg == ptr->firstArg &&
