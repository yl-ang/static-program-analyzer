...
TEST_CASE("FollowsStore - Base Test") {
    ...
    SECTION("Test getFolloweesStar and getFollowersStar") {
        ...
        REQUIRE(followsStore.getFolloweesStar(1) == std::unordered_set<StmtNum>{2, 3, 4});
        REQUIRE(followsStore.getFolloweesStar(2) == std::unordered_set<StmtNum>{3, 4});
        REQUIRE(followsStore.getFolloweesStar(3) == std::unordered_set<StmtNum>{4});
        REQUIRE(followsStore.getFolloweesStar(4).empty());

        REQUIRE(followsStore.getFollowersStar(1).empty());
        REQUIRE(followsStore.getFollowersStar(2) == std::unordered_set<StmtNum>{1});
        REQUIRE(followsStore.getFollowersStar(3) == std::unordered_set<StmtNum>{1, 2});
        REQUIRE(followsStore.getFollowersStar(4) == std::unordered_set<StmtNum>{1, 2, 3});
    }

    SECTION("Test containsFollowRelationship and containsFollowStarRelationship") {
        followsStore.setFollowsStore({{1, 2}, {3, 4}, {5, 6}});
        ...
        REQUIRE(followsStore.containsFollowStarRelationship(1, 4));
        REQUIRE_FALSE(followsStore.containsFollowStarRelationship(2, 1));
        REQUIRE_FALSE(followsStore.containsFollowStarRelationship(3, 5));
    }
}
