...
//         std::unordered_set<std::pair<int, std::string>> usesStoreEntries{std::pair<int, std::string>{2, "x"}, std::pair<int, std::string>{3, "x"}};
//         pfw.setStatementUsesStore(usesStoreEntries);

//         std::unordered_set<std::pair<int, std::string>> modifiesStoreEntries{std::pair<int, std::string>{1, "x"}, std::pair<int, std::string>{2, "x"}};
//         pfw.setStatementModifiesStore(modifiesStoreEntries);

//         Synonym* stmtSyn = new Synonym(DesignEntityType::ASSIGN, "s");

//         // Select s such that Affects(s, 2)
//         AffectsTester{pfr, stmtSyn, new Integer("2")}.testSynonyms({*stmtSyn}).testSynonymValues({{"1"}});
//         // Select s such that Affects(s, 3)
//         AffectsTester{pfr, stmtSyn, new Integer("3")}.testSynonyms({*stmtSyn}).testSynonymValues({{"2"}});
...
//         AffectsTester{pfr, new Integer("2"), stmtSyn}.testSynonyms({*stmtSyn}).testSynonymValues({{"3"}});
//         // Select s such that Affects(3, s)
//         AffectsTester{pfr, new Integer("3"), stmtSyn}.testSynonyms({*stmtSyn}).testSynonymValues({{}});
//         // Select s such that Affects(1, s)
//         AffectsTester{pfr, new Integer("1"), stmtSyn}.testSynonyms({*stmtSyn}).testSynonymValues({{"2"}});

//         // Select s such that Affects(5, s), out of bounds
...
//     }
    ...
    */
    ...
    // SECTION("Affects(synonym, wildcard) / empty store") {
