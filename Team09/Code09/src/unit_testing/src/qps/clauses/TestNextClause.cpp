...
TEST_CASE("SuchThatClause evaluate for Next relationship with no synonyms") {
    ...
    SECTION("Next(Integer, Integer)") {
        ...
        NextTester{pfr, new Integer("1"), new Integer("2")}.testBoolean(true);
        NextTester{pfr, new Integer("2"), new Integer("1")}.testBoolean(false);
        NextTester{pfr, new Integer("1"), new Integer("1")}.testBoolean(false);
        NextTester{pfr, new Integer("2"), new Integer("2")}.testBoolean(false);
    }

    SECTION("Next(Integer, Integer) / empty store") {
        pfw.setNextStore({});
        NextTester{pfr, new Integer("1"), new Integer("2")}.testBoolean(false);
    }

    SECTION("Next(Integer, Wildcard)") {
        std::unordered_set<std::pair<int, int>> nextStoreEntries{std::pair<int, int>{1, 2}};
        pfw.setNextStore(nextStoreEntries);
        NextTester{pfr, new Integer("1"), new Wildcard()}.testBoolean(true);
        NextTester{pfr, new Integer("2"), new Wildcard()}.testBoolean(false);
    }

    SECTION("Next(Integer, Wildcard) / empty store") {
        pfw.setNextStore({});
        NextTester{pfr, new Integer("1"), new Wildcard()}.testBoolean(false);
        NextTester{pfr, new Wildcard(), new Wildcard()}.testBoolean(false);
    }

    SECTION("Next(Wildcard, Integer)") {
        std::unordered_set<std::pair<int, int>> nextStoreEntries{std::pair<int, int>{1, 2}};
        pfw.setNextStore(nextStoreEntries);
        NextTester{pfr, new Wildcard(), new Integer("2")}.testBoolean(true);
        NextTester{pfr, new Wildcard(), new Integer("1")}.testBoolean(false);
    }
}
...
TEST_CASE("SuchThatClause evaluate for Next relationship with 1 synonym") {
    ...
    SECTION("Next(Synonym, Integer)") {
        ...
        NextTester{pfr, stmtSyn, new Integer("2")}.testSynonyms({*stmtSyn}).testSynonymValues({{"1"}});
        NextTester{pfr, stmtSyn, new Integer("1")}.testSynonyms({*stmtSyn}).testSynonymValues({{}});
    }
    ...
    SECTION("Next(Integer, Synonym)") {
        ...
        Synonym* stmtSyn = new Synonym(DesignEntityType::STMT, "s");
        NextTester{pfr, new Integer("1"), stmtSyn}.testSynonyms({*stmtSyn}).testSynonymValues({{"2"}});
        NextTester{pfr, new Integer("2"), stmtSyn}.testSynonyms({*stmtSyn}).testSynonymValues({{}});
    }
    ...
    SECTION("Next(Synonym, Wildcard)") {
        ...
        Synonym* stmtSyn = new Synonym(DesignEntityType::STMT, "s");
        NextTester{pfr, stmtSyn, new Wildcard()}.testSynonyms({*stmtSyn}).testSynonymValues({{"1", "3"}});
    }
    ...
    SECTION("Next(Wildcard, Synonym)") {
        ...
        Synonym* stmtSyn = new Synonym(DesignEntityType::STMT, "s");
        NextTester{pfr, new Wildcard(), stmtSyn}.testSynonyms({*stmtSyn}).testSynonymValues({{"2", "4"}});
    }
}
